...
function AddPC() {
  ...
  useEffect(() => {
    ...
    if (!valid?.validCollaborator) return naviagte('/add-note/collaborator-info');
    handleType('pcData');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const { handleSubmit, loading, error, success } = useForm();
  ...
  return (
    <>
      <form className={styles.addPcForm}>
        ...
        />
        <hr className={styles['col-6']} />
        <DatePicker handleField={handleField} pcData={pcData} />
        <InputField
          id='price'
          label='ლეპტოპის ფასი'
          hint='მხოლოდ ციფრები'
